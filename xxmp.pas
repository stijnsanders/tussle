...
type
  TXxmtussle=class(TXxmProject)//TODO: IXxmProjectEvents1
  public
...
begin
  Result:=TXxmtussle.Create(AProjectName);
end;
...
procedure TXxmtussle.AfterConstruction;
begin
  inherited;
  GameQueue:=TGameQueue.Create;
end;

function TXxmtussle.LoadPage(Context: IXxmContext; const Address: WideString): IXxmFragment;
var
  verb:WideString;
begin
  inherited;
...
function TXxmtussle.LoadFragment(Context: IXxmContext; const Address, RelativeTo: WideString): IXxmFragment;
begin
  Result:=XxmFragmentRegistry.GetFragment(Self,Address,RelativeTo);
end;

procedure TXxmtussle.UnloadFragment(Fragment: IXxmFragment);
begin
  inherited;
